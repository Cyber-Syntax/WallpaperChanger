...
class TestStateLoadSave:
    ...
    def test_load_state_nonexistent_file(self):
        """Test loading nonexistent state file returns initialized state."""
        with tempfile.TemporaryDirectory() as tmpdir:
            state_file = Path(tmpdir) / "nonexistent.json"
            result = load(state_file)
            assert result is not None
            assert "version" in result
            assert result["version"] == CURRENT_STATE_VERSION
    ...
    def test_save_and_load(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert save(state_file, state) is True
    ...
    def test_load_state_corrupted_json(self):
        """Test loading corrupted JSON creates backup and returns initialized state."""
        with tempfile.TemporaryDirectory() as tmpdir:
            state_file = Path(tmpdir) / "state.json"
    ...
    def test_load_state_invalid_schema(self):
        """Test loading state with invalid schema returns initialized state."""
        with tempfile.TemporaryDirectory() as tmpdir:
            state_file = Path(tmpdir) / "state.json"
            ...
            assert result is not None
            assert "version" in result
            assert "current_wallpapers" in result
...
class TestStateUpdate:
    ...
    def test_update_state_basic(self):
        ...
        update(state, wallpaper_paths, monitors)
...
class TestRoundRobinSelection:
    ...
    def test_get_next_wallpaper_initializes_state(self, tmp_path):
        ...
        result = next_wallpaper(test_dir, extensions, state, used_images)
    ...
    def test_get_next_wallpaper_cycles_through_images(self, tmp_path):
        ...
        # First call
        result1 = next_wallpaper(test_dir, extensions, state, used_images)
        assert result1.name == "image1.jpg"

        # Second call
        used_images = []
        result2 = next_wallpaper(test_dir, extensions, state, used_images)
        assert result2.name == "image2.jpg"

        # Third call
        used_images = []
        result3 = next_wallpaper(test_dir, extensions, state, used_images)
        assert result3.name == "image3.jpg"

        # Fourth call should wrap around
        used_images = []
        result4 = next_wallpaper(test_dir, extensions, state, used_images)
    ...
    def test_get_next_wallpaper_handles_directory_changes(self, tmp_path):
        ...
        # First call
        next_wallpaper(test_dir, extensions, state, used_images)
        dir_key = str(test_dir.resolve())
        assert state["round_robin"][dir_key]["position"] == 1
